...
class Perplexity(EvaluationFunction):
    def __call__(self, experiment: Experiment):
        ...
        return experiment.estimator.pdfa.perplexity(
            experiment.X_test,
            experiment.y_test,
            ignore_reject_trace=False) / n_trace
...
class BatchEvaluationFunction(EvaluationFunction):
    """
    This class can contain a multiple different evaluation functions.
    All of the defined metrics are evaluated in batch
    in the ComparativeEvaluation class
    """
    def __init__(self, metrics: Dict[str, EvaluationFunction] = None):
        """
        :param metrics:             A dictionary of a string to an
                                    evaluation function
        """
        if metrics:
...
class ComparativeEvaluation(metaclass=ABCMeta):
    """
    This class evaluates all of the learned automaton
    based on a evaluation function
    """
    ...
    def _read_variables(self, experiment: Experiment) -> Dict:
        """
    ...
    def plot(self,
        ...
        :param columns:             A choice of labels to be plotted
                                    in one figure.
                                    e.g. if ['clf', 'p'] is provided,
                                    Then all the possible combinations
                                    of (clf, p) are plotted as a line
                                    in the plot.
                                    e.g. (Vanilla, 0.6), (Preprocess, 1)
        :param save:                Whether to export plots as files
    ...
    def draw(self,
             ...
             filename: str = None):
        """
        Draw the pdfa with the selected params
