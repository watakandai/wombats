...
class FDFA(Automaton):
    ...
    def load_flexfringe_data(cls: 'FDFA', graph: nx.MultiDiGraph,
        ...
        (nodes,
         node_ID_to_node_label) = cls.convert_flexfringe_nodes(
            ff_nodes,
    ...
    @staticmethod
    def convert_flexfringe_nodes(flexfringe_nodes: dict,
                                 number_input_symbols: int,
                                 root_node_label: Node) -> (NXNodeList, dict):
        """
    ...
    def convert_flexfringe_edges(flexfringeEdges: NXEdgeList,
        ...
        for src_FF_node_ID, dest_FF_node_ID, edge_data in flexfringeEdges:
            ...
            for symbol, frequency in transitionData:
                ...
                new_edge = (src_FF_node_label, dest_FF_node_label,
                            new_edge_data)
    ...
    def _compute_node_data_properties(self, curr_node: Node,
        ...
        number_trans_in = self._compute_node_flow(curr_node,
                                                  flow_type='in')
        number_trans_out = self._compute_node_flow(curr_node,
                                                   flow_type='out')
