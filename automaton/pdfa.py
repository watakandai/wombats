...
class PDFA(Automaton):
    ...
    def score(self, trace: Symbols, epsilon=0.0) -> float:
        """
        ...
        for symbol in trace:
            ...
            try:
                ...
                warnings.warn(str(e))
                # Prevent -np.inf <- log(score=0)
                return epsilon ** n_symbol
    ...
    def logscore(self, trace: Symbols, base: float = 2.0, epsilon: float = 0.0001) -> float:
        """
        ...
        score = self.score(trace, epsilon)
    ...
    def cross_entropy(self, traces: List[Symbols],
        ...
        for target_prob, trace in zip(actual_trace_probs, traces):
            logscore = self.logscore(trace, base, epsilon)
    ...
    def kldivergence(self, traces: List[Symbols],
        ...
        for target_prob, trace in zip(actual_trace_probs, traces):
            logscore = self.logscore(trace)
            target_logscore = np.asscalar(np.log(target_prob) / np.log(base))
            if target_logscore == -np.inf:
                target_logscore = 0.0
            kldivergence += target_prob * logscore - target_prob * target_logscore
        return kldivergence
